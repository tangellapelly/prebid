...
const url = '//pa.publir.com/analytic';
...
let localStoragePrefix = 'publir_analytics_';
...
let publirAdapter = Object.assign(adapter({ url, analyticsType }), {
...
publirAdapter.originEnableAnalytics = publirAdapter.enableAnalytics;

publirAdapter.enableAnalytics = function(config) {
    initOptions = config.options;
    initOptions.utmTagData = this.buildUtmTagData();
    utils.logInfo('Publir Analytics enabled with config', initOptions);
    publirAdapter.originEnableAnalytics(config);
};

publirAdapter.buildUtmTagData = function() {
    let utmTagData = {};
...
function send(eventType, data, sendDataType) {
    ...
    xhr.onreadystatechange = function(result) {
        ...
        utils.logInfo('Event ' + eventType + ' sent ' + sendDataType + ' to publir prebid analytic with result' + result);
    }
...
adaptermanager.registerAnalyticsAdapter({
    adapter: publirAdapter,
    code: 'publir'
});

export default publirAdapter;